...
import { Role, JobStatus, JobPriority, JobType, type User, type Job, type Announcement, type JobHistoryEntry, type StatsResult, type SparePartOrder, SparePartOrderStatus, SparePartOrderPriority, SparePartOrderType } from '../types';
...
export const apiGetJobs = async (
    ...
    // Enrich repair jobs with linked spare part status for branch users
    if (user.role === Role.Branch && jobs.length > 0) {
        const repairJobIds = jobs
            .filter(j => j.job_type === JobType.Reparacion)
            .map(j => j.id);
        ...
        if (repairJobIds.length > 0) {
            const { data: spareParts, error: spError } = await supabase
                ...
                .in('job_id', repairJobIds);

            if (spError) {
                console.error("Error fetching linked spare parts:", spError);
            } else if (spareParts) {
                const sparePartInfoMap = new Map<string, { id: string; status: SparePartOrderStatus }>();
                for (const sp of spareParts) {
                    if (sp.job_id) {
                        sparePartInfoMap.set(sp.job_id, { id: sp.id, status: sp.status as SparePartOrderStatus });
                    }
                }

                jobs = jobs.map(job => {
                    if (sparePartInfoMap.has(job.id)) {
                        const spInfo = sparePartInfoMap.get(job.id)!;
                        return {
                            ...job,
                            linked_spare_part: {
                                id: spInfo.id,
...
export const apiCreateJob = async (jobData: { id: string; description: string; branch_id: string; branch_name: string; job_type: JobType }): Promise<Job> => {
    ...
    const newJob: Omit<Job, 'created_at' | 'updated_at' | 'linked_spare_part'> = {
        ...jobData,
...
// --- Spare Parts ---
export const apiGetSparePartOrders = async (
    user: User,
    options: {
        page?: number;
        pageSize?: number;
        searchTerm?: string;
        sortBy?: 'updated_at' | 'created_at' | 'priority';
        statusFilter?: SparePartOrderStatus | 'ACTIVE' | 'HISTORY';
        branchIdFilter?: string;
        supplierFilter?: string;
        priorityFilter?: SparePartOrderPriority;
        orderTypeFilter?: SparePartOrderType;
    } = {}
): Promise<{ orders: SparePartOrder[]; count: number }> => {
    const { page = 1, pageSize = 15, searchTerm, sortBy = 'created_at', statusFilter, branchIdFilter, supplierFilter, priorityFilter, orderTypeFilter } = options;
    let query = supabase.from('spare_part_orders').select('*', { count: 'exact' });
    ...
    if (statusFilter) {
        if (statusFilter === 'ACTIVE') {
            query = query.in('status', [SparePartOrderStatus.Ordered, SparePartOrderStatus.ReceivedCentral, SparePartOrderStatus.SentToBranch]);
        } else if (statusFilter === 'HISTORY') {
            query = query.in('status', [SparePartOrderStatus.ReceivedByBranch, SparePartOrderStatus.Cancelled]);
        } else {
            query = query.eq('status', statusFilter as SparePartOrderStatus);
        }
    }

    if (searchTerm) {
        query = query.or(`description.ilike.%${searchTerm}%,supplier.ilike.%${searchTerm}%,branch_name.ilike.%${searchTerm}%,order_reference.ilike.%${searchTerm}%,notes.ilike.%${searchTerm}%,requested_by.ilike.%${searchTerm}%,job_id.ilike.%${searchTerm}%`);
    }
    ...
    if (error) throw error;
    return { orders: (data as SparePartOrder[]) || [], count: count ?? 0 };
};

export const apiCreateSparePartOrder = async (orderData: Omit<SparePartOrder, 'id'|'created_at'|'updated_at'|'status'|'history'>, createdBy: User): Promise<SparePartOrder> => {
    // Backend validation to prevent duplicate active orders for the same job
    if (orderData.job_id) {
        const { data: existingOrders, error: checkError } = await supabase
            .from('spare_part_orders')
            .select('id')
            .eq('job_id', orderData.job_id)
            .in('status', [SparePartOrderStatus.Ordered, SparePartOrderStatus.ReceivedCentral, SparePartOrderStatus.SentToBranch]);
        
        if (checkError) throw checkError;

        if (existingOrders && existingOrders.length > 0) {
            throw new Error(`El trabajo #${orderData.job_id} ya tiene un pedido de repuesto activo.`);
        }
    }
...
export const apiUpdateSparePartOrder = async (orderId: string, updates: Partial<Omit<SparePartOrder, 'id' | 'history'>>, updatedBy: User): Promise<SparePartOrder> => {
    const { data: currentOrder, error: fetchError } = await supabase.from('spare_part_orders').select('*').eq('id', orderId).single();
    if (fetchError) throw fetchError;
    if (!currentOrder) throw new Error("Pedido de repuesto no encontrado");
    
    // Backend validation to prevent linking to a job that already has an active order
    if (updates.job_id && updates.job_id !== currentOrder.job_id) {
        const { data: existingOrders, error: checkError } = await supabase
            .from('spare_part_orders')
            .select('id')
        ...
        if (existingOrders && existingOrders.length > 0) {
            throw new Error(`El trabajo #${updates.job_id} ya tiene un pedido de repuesto activo.`);
        }
    }
    ...
    if (updates.notes && updates.notes !== currentOrder.notes) {
        newHistory.push({ timestamp: now, status: "Nota añadida", notes: updates.notes, updatedBy: updatedBy.username });
    }
    ...
    if (updates.order_type && updates.order_type !== currentOrder.order_type) {
        newHistory.push({ timestamp: now, status: `Tipo cambiado a ${updates.order_type}`, updatedBy: updatedBy.username });
    }

    if ('order_reference' in updates && updates.order_reference !== currentOrder.order_reference) {
        const note = updates.order_reference 
            ? `Nº de pedido/referencia actualizado a: ${updates.order_reference}`
            : 'Nº de pedido/referencia eliminado.';
        newHistory.push({ timestamp: now, status: `Referencia actualizada`, notes: note, updatedBy: updatedBy.username });
    }

    if ('job_id' in updates && updates.job_id !== currentOrder.job_id) {
        const note = updates.job_id
            ? `Vinculado al trabajo #${updates.job_id}`
            : 'Desvinculado de trabajo.';
        newHistory.push({ timestamp: now, status: `Vínculo actualizado`, notes: note, updatedBy: updatedBy.username });
    }
...
export const apiDeleteSparePartOrder = async (orderId: string): Promise<{ success: true }> => {
    const { error } = await supabase.from('spare_part_orders').delete().eq('id', orderId);
    if (error) throw error;
    return { success: true };
};

export const apiGetOpenRepairJobsForBranch = async (branchId: string): Promise<Pick<Job, 'id' | 'description'>[]> => {
    const { data, error } = await supabase
        .from('jobs')
        .select('id, description')
        .eq('branch_id', branchId)
        .eq('job_type', JobType.Reparacion)
        .not('status', 'eq', JobStatus.ReceivedByBranch);

    if (error) throw error;
    return data || [];
};

export const apiGetRepairsWaitingForParts = async (): Promise<Job[]> => {
    // 1. Find all active spare part orders that are linked to a job
    const activeStatuses = [
        SparePartOrderStatus.Ordered,
        SparePartOrderStatus.ReceivedCentral,
        SparePartOrderStatus.SentToBranch
    ];

    const { data: activeSpareParts, error: spError } = await supabase
        .from('spare_part_orders')
        .select('id, job_id, status')
    ...
    if (!activeSpareParts || activeSpareParts.length === 0) {
        return [];
    }

    const jobIds = activeSpareParts.map(sp => sp.job_id).filter((id): id is string => !!id);
    if (jobIds.length === 0) {
        return [];
    }

    const sparePartInfoMap = new Map<string, { id: string, status: SparePartOrderStatus }>();
    activeSpareParts.forEach(sp => {
        if (sp.job_id) {
            sparePartInfoMap.set(sp.job_id, { id: sp.id, status: sp.status as SparePartOrderStatus });
        }
    });

    // 2. Fetch the corresponding repair jobs
    ...
    // 3. Enrich jobs with the spare part status
