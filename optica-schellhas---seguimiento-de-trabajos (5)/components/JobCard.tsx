...
import { AlertTriangleIcon, BranchIcon, CheckCircleIcon, SendIcon, TruckIcon, CheckIcon, RepeatIcon, WrenchIcon, PackageIcon } from './common/Icons';
...
const JobCard: React.FC<JobCardProps> = ({ job, user, onUpdate, onUpdatePriority, onViewHistory, isSelected, onSelect, selectable = true }) => {
    ...
    const renderActions = () => {
        if (user.role === Role.Branch) {
            if (job.status === JobStatus.PendingInBranch) {
                if (isWaitingForPart) {
                    return null; // Do not show the button if waiting for a part
                }
                return <Button size="sm" variant="primary" onClick={handleActionClick}>
                    <SendIcon className="h-4 w-4 mr-2"/> Enviar a Laboratorio
                </Button>;
            }
            if (job.status === JobStatus.SentToBranch) {
                return <Button size="sm" variant="primary" onClick={handleActionClick}><CheckCircleIcon className="h-4 w-4 mr-2"/> Marcar como Recibido</Button>;
            }
        }
                        ...
                        <div className="flex items-start space-x-3">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-800 dark:text-slate-100 flex items-center">
                                    {job.job_type === JobType.Reparacion && <WrenchIcon className="h-4 w-4 mr-2 text-gray-500" title="Reparación"/>}
                                    Trabajo #{job.id}
                                </h3>
                                <p className="text-sm text-gray-500 dark:text-slate-400">{job.description || 'Sin descripción'}</p>
                                {isWaitingForPart && (
                                    <div className="mt-2 text-xs font-semibold text-amber-600 dark:text-amber-400 flex items-center">
                                        <PackageIcon className="h-4 w-4 mr-1.5" />
                                        Esperando repuesto: {job.linked_spare_part?.status}
                                    </div>
                                )}
                            </div>
                        </div>
                       ...
                       <div>
                            <div className="flex items-center text-sm text-gray-600 dark:text-slate-300">
                             ...
                             <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">
                                Última act.: {new Date(job.updated_at).toLocaleString('es-ES')}
                            </p>
                       </div>
