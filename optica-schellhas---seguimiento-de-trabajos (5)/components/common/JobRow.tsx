...
import { AlertTriangleIcon, CheckCircleIcon, SendIcon, TruckIcon, CheckIcon, RepeatIcon, ChevronRightIcon, HistoryIcon, WrenchIcon } from './Icons';
...
export const JobRow: React.FC<JobRowProps> = ({ job, user, onUpdate, onUpdatePriority, onViewHistory, isSelected, onSelect, selectable = true }) => {
    ...
    const renderActions = () => {
        if (user.role === Role.Branch) {
            if (job.status === JobStatus.PendingInBranch) return <Button size="sm" variant="secondary" onClick={handleActionClick}><SendIcon className="h-4 w-4 mr-2"/>Enviar</Button>;
