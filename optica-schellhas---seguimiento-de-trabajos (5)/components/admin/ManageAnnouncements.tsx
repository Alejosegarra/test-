...
const ManageAnnouncements: React.FC<{announcements: Announcement[], onUpdate: () => void}> = ({announcements, onUpdate}) => {
    ...
    return (
        <Card>
             <CardHeader>
                 <h2 className="text-xl font-bold text-gray-800 dark:text-slate-100 flex items-center"><MegaphoneIcon className="h-6 w-6 mr-2 text-gray-600 dark:text-slate-400"/>Gestionar Anuncios</h2>
             </CardHeader>
             <CardContent>
                ...
                <ul className="space-y-3">
                    {announcements.map(ann => (
                        <li key={ann.id} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
                            <p className="text-gray-800 dark:text-slate-200">{ann.message}</p>
                            <Button variant="ghost" size="sm" onClick={() => handleDelete(ann.id)}><TrashIcon className="h-5 w-5 text-red-500"/></Button>
                        </li>
                    ))}
                    {announcements.length === 0 && <p className="text-gray-500 dark:text-slate-400 text-center py-4">No hay anuncios activos.</p>}
                </ul>
             </CardContent>
        </Card>
    )
