...
import { Card, CardHeader, CardContent, Button, Input, Select, Checkbox, DateInput } from '../common/UI';
...
import ManageSpareParts from './ManageSpareParts';
import { BriefcaseIcon, HistoryIcon, RepeatIcon, BranchIcon, InboxIcon, AlertTriangleIcon, ClockIcon, XIcon, PackageIcon } from '../common/Icons';
...
type AdminView = 'dashboard' | 'jobs' | 'accounts' | 'announcements' | 'spare_parts';
type RawJobForStats = Pick<Job, 'branch_name' | 'priority' | 'created_at' | 'status' | 'history'>;
type HealthStats = { pendingReceiptInLab: number; activeAlerts: number; maxLabAgeHours: number; overdueJobs: number; };
...
const AdminPanel: React.FC<{user: User, announcements: Announcement[], onAnnouncementsUpdate: () => void}> = ({user, announcements, onAnnouncementsUpdate}) => {
    ...
    const chartDataCycleTime = useMemo(() => ({
        ...
        datasets: [{
            ...
            // FIX: Explicitly cast `t` to a number to resolve TypeScript error where
            // Object.values was being inferred as `unknown[]`.
            data: Object.values(stats.cycleTimeByBranch).map(t => parseFloat((t as number).toFixed(1))),
    ...
    const chartDataTimeInStatus = useMemo(() => {
        ...
        return {
            ...
            datasets: [{
                ...
                data: orderedStatuses.map(status => parseFloat((Number(data[status]) || 0).toFixed(1))),
    ...
    return (
        <div>
            <div className="flex border-b border-gray-200 dark:border-slate-700 mb-6">
                {(['dashboard', 'jobs', 'spare_parts', 'accounts', 'announcements'] as AdminView[]).map(v => (
                    <button key={v} onClick={() => setView(v)} className={`capitalize py-2 px-4 text-sm md:text-base font-medium ${view === v ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 dark:text-slate-400'}`}>
                        {v === 'dashboard' ? 'Métricas' : v === 'jobs' ? 'Trabajos' : v === 'spare_parts' ? 'Repuestos' : v === 'accounts' ? 'Cuentas' : 'Anuncios'}
                    </button>
                ))}
            </div>
            ...
            {view === 'dashboard' && (
                <div>
                     ...
                     <div className="p-4 mb-6 bg-gray-50 dark:bg-slate-800/50 rounded-lg border dark:border-slate-700">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                            <div>
                                ...
                                <DateInput value={dateRange.startDate} onChange={(value) => handleDateChange('startDate', value)} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Hasta</label>
                                <DateInput value={dateRange.endDate} onChange={(value) => handleDateChange('endDate', value)} />
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                        ...
                        <StatCard title="Trabajos Demorados (+48hs)" value={healthStats?.overdueJobs?.toString() ?? '0'} icon={<AlertTriangleIcon />} colorClass="text-orange-500" threshold={{value: 1, className: 'bg-orange-100 dark:bg-orange-900/30 border border-orange-300 dark:border-orange-700'}} />
                    </div>
            ...
            {view === 'spare_parts' && <ManageSpareParts user={user} />}
