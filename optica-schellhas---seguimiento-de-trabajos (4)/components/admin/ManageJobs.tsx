...
import { BriefcaseIcon, PlusIcon, EditIcon, TrashIcon, HistoryIcon } from '../common/Icons';
import { Pagination } from '../common/Pagination';
import { JobHistoryModal } from '../common/JobHistoryModal';

const PAGE_SIZE = 15;

const JobRowCard: React.FC<{ job: Job, onEdit: (job: Job) => void, onDelete: (job: Job) => void, onViewHistory: (job: Job) => void }> = ({ job, onEdit, onDelete, onViewHistory }) => (
    <Card className="mb-4">
        <CardContent>
            <div className="flex justify-between items-start">
                ...
                <div className="text-right space-x-1">
                    <Button variant="ghost" size="sm" onClick={() => onViewHistory(job)} title="Ver historial">
                ...
                <CardContent>
                    ...
                    <>
                        ...
                        <div className="md:hidden">
                            {jobs.length > 0 ? (
                                jobs.map(job => (
                                    <JobRowCard key={job.id} job={job} onEdit={handleOpenModal} onDelete={handleOpenDeleteModal} onViewHistory={setViewingHistoryJob} />
                                ))
                            ) : (
                                    ...
                                    {jobs.length > 0 ? jobs.map(job => (
                                        <tr key={job.id} className="border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                            ...
                                            <td className="p-3 text-right space-x-1">
                                                <Button variant="ghost" size="sm" onClick={() => setViewingHistoryJob(job)} title="Ver historial">
                                                    <HistoryIcon className="h-5 w-5 text-gray-600 dark:text-slate-300"/>
                                                </Button>
