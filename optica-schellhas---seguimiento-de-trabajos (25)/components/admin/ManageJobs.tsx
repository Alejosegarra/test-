...
import { Role, JobStatus, JobPriority, type User, type Job } from '../../types';
        ...
        <CardContent>
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-gray-800 font-semibold">{job.description || "Sin descripción"}</p>
                </div>
...
const ManageJobs: React.FC<{ user: User }> = ({ user }) => {
    ...
    const fetchAllData = useCallback(async () => {
        ...
        try {
            const [jobsResponse, branchesData] = await Promise.all([
                api.apiGetJobs(user, { page: currentPage, pageSize: PAGE_SIZE, searchTerm }),
    ...
    }, [user, addToast, currentPage, searchTerm]);
        ...
        <Card>
            <CardHeader className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div className="flex-1">
                    <h2 className="text-xl font-bold text-gray-800 flex items-center"><BriefcaseIcon className="h-6 w-6 mr-2 text-gray-600"/>Gestionar Trabajos</h2>
                    <Input 
                        value={searchTerm} 
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b">
                                    ...
                                    <th className="p-3 font-semibold text-gray-700">Prioridad</th>
                            ...
                            <tbody>
                                {jobs.length > 0 ? jobs.map(job => (
                                    <tr key={job.id} className="border-b hover:bg-gray-50">
                                        <td className="p-3 font-mono text-gray-900">{job.id}</td>
                                        <td className="p-3 text-gray-800">{job.description}</td>
                                        <td className="p-3 text-gray-600">{job.branch_name}</td>
                                        <td className="p-3 text-gray-600">{job.status}</td>
