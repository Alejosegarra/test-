...
type AdminView = 'dashboard' | 'jobs' | 'accounts' | 'announcements' | 'reports';
    ...
    return (
        <div>
            <div className="flex border-b border-gray-200 dark:border-slate-700 mb-6 overflow-x-auto">
                {(['dashboard', 'jobs', 'accounts', 'announcements', 'reports'] as AdminView[]).map(v => (
                    <button key={v} onClick={() => setView(v)} className={`capitalize py-2 px-4 text-sm md:text-base font-medium whitespace-nowrap ${view === v ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 dark:text-slate-400'}`}>
                        {v === 'dashboard' ? 'Métricas' : v === 'jobs' ? 'Trabajos' : v === 'accounts' ? 'Cuentas' : v === 'announcements' ? 'Anuncios' : 'Reportes'}
                    </button>
                ))}
            </div>
            ...
            {view === 'reports' && <ManageReports user={user} />}
        </div>
    );
};
